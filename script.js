...
const activeTasks = localStorage.getItem("activeTasks") ? JSON.parse(localStorage.getItem("activeTasks")) :
[];
...
const renderTodos = () => {
    ...
    if (completedTasks){
        ...
        completedTasks.map((task, index) => {
            const divtoInsert = document.createElement("div");
            divtoInsert.setAttribute("id", index)
            divtoInsert.innerHTML = `<div key=${index} style="display: flex; justify-content: space-between">
            <div>
...
activeNavlink.addEventListener(
    "click", () => toggleVisibility(activeDiv)
)

allNavlink.addEventListener(
    "click", () => toggleVisibility(allDiv)
)

completedNavlink.addEventListener(
    "click", () => toggleVisibility(completedDiv)
);
